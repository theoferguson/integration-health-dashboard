...
export const RECOMMENDED_WORKFLOW: string[] = [
  ...
  'A `stale` integration has stopped reporting - do not go looking for a failing event to explain it, there will not be one. Its most recent events may all be successes; the problem is that they stopped. Check `lastSync` against `expectedIntervalMs` to see how long it has been gone.',
...
The unit of data is an **event**: one integration (\`weather\`, \`stripe\`, ...) reported one occurrence (\`forecast.sync\`) that succeeded or failed, with an optional payload, error, and reporter-supplied metrics and tags. Integrations are discovered from the events themselves - there is no static registry, so the set of integration ids differs per organization. Health is derived per integration from success rate and error count over a rolling 24h window, plus whether it is still reporting at all, and reduces to \`healthy\`, \`degraded\`, or \`down\`.

Silence is a first-class failure here, and it is the one a success rate cannot express: an integration can have succeeded on every event it ever sent and still be dead right now. Each integration carries \`expectedIntervalMs\` (how long it normally goes between events, inferred from its own history - nothing declares a schedule) and \`stale\`. An integration silent for more than 3x its normal gap is \`stale\` and at least \`degraded\`; past 10x it is \`down\`. Trust \`stale\` rather than re-deriving it from \`lastSync\`. \`lastSync\` is the last event ever, including outside the 24h window, so it tells you when the silence started. \`successRate\` is null when nothing arrived in the window - no events means no rate, not a perfect one.
